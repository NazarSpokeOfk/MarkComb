...
class CodeInputFunctions {
    handleChange = <T,>({index, code , values , setValues , onComplete , inputsRef , setData} : HandleChangeCodeInputProps<T>) => {
        ...
        if (newValues.every((val) => val !== "")) {
          ...
          setData((prev) => ({...prev, verification_code : newValues.join("")}))
        }
      };
