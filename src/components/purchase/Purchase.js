...
const Purchase = (isLoggedIn) => {
  ...
  useEffect(()=>{
    //Нужно переписать на другой формат
      ...
      <section className="balance">
        <div className="container">
          ...
          <div className="balance__block">
            <h3 className="balance-block__money">{ isLoggedIn.isLoggedIn ? '$100' : 'Firstly,log in to your account'}</h3>
            <h3 className="balance-block__uses">{ isLoggedIn.isLoggedIn ? '100 uses' : ''}</h3>
            <div className="balance-block__variants">
              <button className="variant">
                <img
                  src={payPic}
                  alt="bank card"
                  className="variant__img"
                />
              </button>
              <button className="variant">
                <img
      ...
      <section className="packages">
        ...
        <div className="packages__wrapper">
          <div className="packages-light__package">
            ...
            <button onClick = {()=> handleClick()} className="package__button none">
              {t('purch')}<span>{t('ase')}</span>
            </button>
          </div>
          ...
          <div id="business" className="packages-light__package">
            ...
            <h4 className="package__usages-business none">5 {t('uses(s)')} / {t('day')}</h4>
            ...
            <button onClick = {()=> handleClick()} className="package__button-bussines none">
            {t('purch')}<span>{t('ase')}</span>
            </button>
          </div>
        </div>
